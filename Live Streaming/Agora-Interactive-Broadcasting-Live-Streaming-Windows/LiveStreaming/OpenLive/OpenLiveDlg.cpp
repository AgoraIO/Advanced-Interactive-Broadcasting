...
BEGIN_MESSAGE_MAP(COpenLiveDlg, CDialogEx)
	...
	ON_MESSAGE(WM_SETPUBLISH_RTMP, &COpenLiveDlg::OnSetPublishRtmp)
...
LRESULT COpenLiveDlg::OnJoinChannel(WPARAM wParam, LPARAM lParam)
{
	...
	lpRtcEngine->setVideoProfile((VIDEO_PROFILE_TYPE)nVideoSolution, false);
	...
	lpAgoraObject->JoinChannel(strChannelName, 0);
...
LRESULT COpenLiveDlg::OnLeaveChannel(WPARAM wParam, LPARAM lParam)
{
	...
	int ret = lpAgoraObject->RemovePublishStreamUrl(m_rtmp_url.c_str());
...
LRESULT COpenLiveDlg::OnSetPublishRtmp(WPARAM wParam, LPARAM lParam)
{
	CPublisherDlg   publishdlg;
	if (IDOK == publishdlg.DoModal()){
		...
		m_dlgVideo.waterMarkUrl = publishdlg.waterFilePath;
	}
	return 0;
}
