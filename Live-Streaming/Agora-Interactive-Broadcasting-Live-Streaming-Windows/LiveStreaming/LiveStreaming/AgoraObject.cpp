...
BOOL CAgoraObject::SetVideoProfile(int nVideoResolutinIndex, BOOL bIsSwitchWH)
{
	int res = m_lpAgoraEngine->setVideoProfile(VIDEO_PROFILE_TYPE(nVideoResolutinIndex),bIsSwitchWH);
...
BOOL CAgoraObject::JoinChannel(LPCTSTR lpChannelName, UINT nUID, LPCSTR lpDynamicKey)
{
	...
	LPCSTR lpStreamInfo = nullptr;// "{\"owner\":true,\"width\":640,\"height\":480,\"bitrate\":500}";
#ifdef UNICODE
...
bool CAgoraObject::enablePublish(bool enable /*= true*/)
{
	...
	publishparam.publishUrl = m_publishParam.rtmpUrl.data();
	publishparam.extraInfo = "{\"lowDelay\":true}";
	publishparam.lifecycle = RTMP_STREAM_LIFE_CYCLE_BIND2CHANNEL;
	publishparam.defaultLayout = 1;

	if (enable){
		publishparam.owner = true;
	}
	else{
		publishparam.owner = false;
	}
